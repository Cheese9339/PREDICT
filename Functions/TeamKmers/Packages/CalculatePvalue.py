            ...
            if K1.split("_")[0] != K2.split("_")[0] and K1.split("_")[1] == RC(K2.split("_")[1]):
                ...
                if Data == "TP":
                    ...
                    if CS > TPCSThreshold:
                        ...
                        # if K1 == "t_TATATAT" and K2 == "t_TATATATA":
                        #     print(K1_counts, sum(K1_counts))
                        #     print(K2_counts, sum(K2_counts))
                        #     print(np.mean(K1_counts, K2_counts), sum(np.mean(K1_counts, K2_counts)))
                        ...
                        TPKmerTotalcount_DF = TPKmerTotalcount_DF.append(ToAppendSeries)
                elif Data == "TN":
                    FlattenScaled_K1_counts = Scaled_K1_counts.flatten()
                    FlattenScaled_K2_counts = Scaled_K2_counts.flatten()
                    K1K2_IntersctionMin = pd.Series((FlattenScaled_K1_counts + FlattenScaled_K2_counts) / 2)
...
def main(new_folder, TPGeneXKmerDF, TNGeneXKmerDF, pThreshold=0.99, Comparison="Greater", TPCSThreshold=0.75, TPTNCSThreshold=0.3):
